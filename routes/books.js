...
import { Book, validateCreateBook, validateUpdateBook } from '../schemas/Book.js'
 ...
 */
router.post('/', verifyAdmin, asyncHandler(
  async(req, res) => {
 ...
 * @access private (only admin)
 */
router.put('/:id', verifyAdmin, asyncHandler(
  async (req, res) => {
    ...
    const result = await Book.updateOne({ _id: req.params.id }, { $set: req.body });
    res.status(201).json(result);
  }
))
 ...
 * @access private (only admin)
 */
router.delete('/:id', verifyAdmin, asyncHandler(
  async (req, res) => {
    const book = await Book.findById(req.params.id);
    if (!book) {
      res.status(404).json({ message: 'Book doesn\'t exist' });
